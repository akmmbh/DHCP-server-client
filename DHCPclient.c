...
int main(int argc,char *argv[]){
    ...
    int s,i,slen= sieof(si_other),recv_len,portno;
    ...
    memset((char*)&si_other,0,sieof(si_other));
    ...
    printf("\nBegin DHCP 4-Handshacke -Discover client arrives");
    printf("yiaddr:%s\n",yiaddr);
    //intialie rnadom number generator 
    ...
    if(sendto(s,&yiaddr,sizeof(yiaddr),0(struct sockaddr *)&si_other,slen)==-1)die("sendto()");
    if(sendto(s,&transID,sizeof(transID),0,(struct sockaddr * )&si_other,slen)==-1)die("sendto()");

    //Begin Sequence for DHCP offer
    //Receive from server,print error if it doesnot work
    if((recv_len=recvfrom(s,&yiaddr,sizeof(yiaddr),0,(struct sockaddr *)&si_other,&slen))==-1)die("recvfrom()");
    if((recv_len=recvfrom(s,&transID,sizeof(transID),0,(struct sockaddr *)&si_other,&slen))==-1)die("recvfrom()");
    if((recv_len=recvfrom(s,&lifeTime,sizeof(lifeTime),0,(struct sockaddr *)&si_other,&slen))==-1)die("recvfrom()");
    ...
    if(sendto(s,&yiaddr,sizeof(yiaddr),0,(struct sockaddr *)&si_other,slen)==-1)die("sendto()");
    if(sendto(s,&transID,sizeof(transID),0,(struct sockaddr*)&si_other,slen)==-1)die("sendto()");
    if(sendto(s,&lifeTime,sizeof(lifeTime),0,(struct sockaddr *)&si_other,slen)==-1)("sendto()");

    //Begin sequence for DHCP acknowledge -Receive acknowlegement form DHCP server
